...
public class QSearch
  { private const int MAX_COORD = 10; // Size of maze
    ...
    private void addPosition(int row, int col, int pathl)
    ...
    public void SolveMaze ()
      { int r, c = 0; // Row and column coordinates;
        ...
        while (! posQueue.IsEmpty())
          { Position nextPos;
            ...
            if (maze[r - 1, c] && !beenThere[r - 1, c])
                addPosition(r - 1, c, pathl+1);
            // Try to move right
            if (maze[r, c+1] && ! beenThere[r, c+1])
              addPosition(r, c+1, pathl+1);
            // Try to move down
            if (maze[r+1, c] && ! beenThere[r+1, c])
              addPosition(r+1, c, pathl+1);
            // Try to move left
        ...
        if (c == MAX_COORD - 1) // Found exit
        {
            ...
            Console.WriteLine("Position total: " + position_total);
            Console.WriteLine("The path length is: " + pathl );
        }
        else
